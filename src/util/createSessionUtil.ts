...
import { create, SocketState, Wid } from '@wppconnect-team/wppconnect';
...
export default class CreateSessionUtil {
  ...
  async createSessionUtil(req: any, clientsArray: any, session: any, res: any) {
    try {
      let client: ClientWhatsAppTypes = this.getClient(session);
      ...
      let wppClient = await create(
        Object.assign({}, { tokenStore: myTokenStore }, req.serverOptions.createOptions, {
          ...
          catchQR: (base64Qr: string, asciiQR: string, attempt: string, urlCode: string) => {
            this.exportQR(req, base64Qr, urlCode, client, res);
          },
          onLoadingScreen: (percent: number, message: Message) => {
  ...
  exportQR(req: any, qrCode: string, urlCode: string, client: ClientWhatsAppTypes, res: any) {
  ...
  async onParticipantsChanged(req: any, client: ClientWhatsAppTypes) {
  ...
  async start(req: any, client: ClientWhatsAppTypes) {
    try {
      await client.isConnected();
  ...
  async checkStateSession(client: ClientWhatsAppTypes, req: any) {
  ...
  async listenMessages(client: ClientWhatsAppTypes, req: any) {
    await client.onMessage(async (message: Message) => {
      eventEmitter.emit(`mensagem-${client.session}`, client, message);
      callWebHook(client, req, 'onmessage', message);
      if (message.type === 'location') {
        const id = message.sender.id as unknown as string;
        client.onLiveLocation(id, (location) => {
          callWebHook(client, req, 'location', location);
        });
      }
    });

    await client.onAnyMessage((message: Message) => {
      const newmsg: any = message;
      newmsg.session = client.session;

      if (message.type === 'sticker') {
        download(message, client, req.logger);
      }

      req.io.emit('received-message', { response: newmsg });
    });
  ...
  async listenAcks(client: ClientWhatsAppTypes, req: any) {
  ...
  async onPresenceChanged(client: ClientWhatsAppTypes, req: any) {
  ...
  async onReactionMessage(client: ClientWhatsAppTypes, req: any) {
    await client.isConnected();
  ...
  decodeFunction(text: string, client: ClientWhatsAppTypes) {
