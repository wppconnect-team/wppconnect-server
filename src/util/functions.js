...
export async function callWebHook(client, req, event, data) {
    if (client && client.webhook) {
        ...
        try {
            const chatId = data.from || data.chatId || data.chatId._serialized;
...
async function sendUnread(client, req) {
    ...
    try {
        ...
        if (chats && chats.length > 0) {
            for (let i = 0; i < chats.length; i++) callWebHook(client, req, 'unreadmessages', chats[i]);
        }
