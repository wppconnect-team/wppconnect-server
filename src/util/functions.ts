...
export function contactToArray(number: string, isGroup?: boolean) {
  let localArr = [];
...
export function groupToArray(group: string | string[]) {
  let localArr = [];
...
export function groupNameToArray(group: string | string[]) {
  let localArr = [];
  if (Array.isArray(group)) {
...
export async function callWebHook(client: ClientWhatsAppTypes, req: any, event: any, data: any) {
...
async function autoDownload(client: ClientWhatsAppTypes, req: any, message: any) {
  try {
...
export async function startHelper(client: ClientWhatsAppTypes, req: any) {
...
async function sendUnread(client: ClientWhatsAppTypes, req: any) {
  req.logger.info(`${client.session} : Inicio enviar mensagens não lidas`);

  try {
    const chats = (await client.getAllChatsWithMessages(true)) as any;
...
async function archive(client: ClientWhatsAppTypes, req: any) {
    ...
    if (chats && Array.isArray(chats) && chats.length > 0) {
      for (let i = 0; i < chats.length; i++) {
        ...
        if (DaysBetween(date) > req.serverOptions.archive.daysToArchive) {
          await client.archiveChat(chats[i].id._serialized, true);
...
export function setMaxListners(serverOptions: any) {
